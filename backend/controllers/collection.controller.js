...
const findMatch = asyncHandler(async (req, res) => {
    ...
    if (!selfiePhotoIds || !Array.isArray(selfiePhotoIds) || selfiePhotoIds.length === 0) {
    ...
    if (photos.length !== selfiePhotoIds.length) {
        throw new ApiError(404, "One or more photos not found for the given event");
    }

    // Track guest access
    if (req.user) {
        const existingGuest = await Guest.findOne({ eventId, userId: req.user._id });
        if (existingGuest) {
    ...
    if (allFaces.length === 0) {
        throw new ApiError(404, "No faces detected in the provided photos. Please try with clearer photos.");
    }

    // Perform Match
    const searches = await Promise.all(
        allFaces.map(face => qdrant.search(`Event_${eventId}`, {
